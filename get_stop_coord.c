...
int main(int argc,char**argv){
    ...
    int *featcn=(int*)malloc(sizeof(int)*length);
    ...
    while(1){
        ...
        for(int i=0;i<length;i++){
            if(strcmp(annotation[i].transcript_id,ENST)==0){
                ...
                flag++;
                for(int j=0;j<200;j++){
                    if((annotation[i].stop_coord-101+j)<0)
                        outcoverage[j]=0;
                    else if((annotation[i].stop_coord-101+j)>=translen)
                        outcoverage[j]=0;
                    else
                        outcoverage[j]=coverage[annotation[i].stop_coord-101+j];
                }
                fprintf(out2,"%s",ENST);
                //printf("%s\n",ENST);
